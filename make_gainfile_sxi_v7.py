...
Description:
29/9/23 - Longer Time range to cover all times of Andy R full orbit sims (V7)

12/5/2023 - Increased trap density 10x to test CTI corrections
...
# Initialise and fill the gain cti parameters, 40 time entries
n_time_entries = 40

met = np.zeros(n_time_entries)
met[0] = 695451200.00000000
step = 3.0E+6
for n in range(40):
    met[n] = met[0] + n * step
...
# Set values as 10xBOL
trap_density_image_value = 0.003 * 10.0
trap_density_serial_value = 0.01 * 10.0

trap_density_image_py = np.full(n_time_entries, trap_density_image_value)
trap_density_serial_py = np.full(n_time_entries, trap_density_serial_value)
...
gain_ccdpy_e_value = 1.6

gain_ccdpy_e = np.full(n_time_entries, gain_ccdpy_e_value )
gain_ccdpy_f = gain_ccdpy_e
gain_ccdny_e = gain_ccdpy_e
gain_ccdny_f = gain_ccdpy_e
...
cti_alpha_p_ccdpy_e = np.full(n_time_entries, cti_alpha_value)
cti_alpha_p_ccdpy_f = np.full(n_time_entries, cti_alpha_value)
cti_alpha_p_ccdny_e = np.full(n_time_entries, cti_alpha_value)
cti_alpha_p_ccdny_f = np.full(n_time_entries, cti_alpha_value)

cti_beta_p_ccdpy_e = np.full(n_time_entries, cti_beta_value)
cti_beta_p_ccdpy_f = np.full(n_time_entries, cti_beta_value)
cti_beta_p_ccdny_e = np.full(n_time_entries, cti_beta_value)
cti_beta_p_ccdny_f = np.full(n_time_entries, cti_beta_value)

cti_alpha_s_ccdpy_e= np.full(n_time_entries, cti_alpha_value)
cti_alpha_s_ccdpy_f= np.full(n_time_entries, cti_alpha_value)
cti_alpha_s_ccdny_e= np.full(n_time_entries, cti_alpha_value)
cti_alpha_s_ccdny_f= np.full(n_time_entries, cti_alpha_value)

cti_beta_s_ccdpy_e = np.full(n_time_entries, cti_beta_value)
cti_beta_s_ccdpy_f = np.full(n_time_entries, cti_beta_value)
cti_beta_s_ccdny_e = np.full(n_time_entries, cti_beta_value)
cti_beta_s_ccdny_f = np.full(n_time_entries, cti_beta_value)
...
gpyhdu.header.set('CREATOR', creator, 'calcpi person')
gpyhdu.header.set('VERSION', 'v7', 'Extension version number')
gpyhdu.header.set('FILENAME', 'sxi_gain_v7.fits', 'File Name')
...
gnyhdu.header.set('VERSION', 'v7', 'Extension version number')
gnyhdu.header.set('FILENAME', 'sxi_gain_v7.fits', 'File Name')
...
fitsobj.writeto('sxi_gain_v7.fits')
